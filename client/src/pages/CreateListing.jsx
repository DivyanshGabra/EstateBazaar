...
const CreateListing = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if(formData.imageUrls.length < 1) return setError("You must upload atleast 1 image")
        if(formData.discountedPrice > formData.regularPrice) return setError("Discount Price must be lower than Regular Price")
      setLoading(true)
      setError(false)
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto overflow-hidden">
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {formData.imageUrls.length > 0 &&
            formData.imageUrls.map((url, index) => (
              <div
                className="flex justify-between p-3 border border-3 items-center"
                key={index}
              >
                <img
                  src={url}
                ...
                <button
                  ...
                  type="button"
                  className="text-red-700 p-3 rounded-sm uppercase hover:opacity-85"
                >
                  Delete
                </button>
              </div>
            ))}
          <button disabled = {loading || uploading} className="p-3 text-white bg-slate-700 rounded-sm uppercase hover:opacity-95 disabled:opacity-80">
